...
//    RunwarLogger REQUEST_DUMPER_LOGGER = Logger.getMessageLogger(RunwarLogger.class, RunwarLogger.class.getPackage().getName() + ".request.dump");
    ...
    // @LogMessage(level = Logger.Level.ERROR)
    // @Message(id = 5025, value = "Could not initiate SPDY connection and no
    // HTTP fallback defined")
    // void couldNotInitiateSpdyConnection();
    //
    // @LogMessage(level = INFO)
    // @Message(id = 5026, value = "Jetty ALPN support not found on boot class
    // path, %s client will not be available.")
    ...
    // @LogMessage(level = Logger.Level.ERROR)
    // @Message(id = 5030, value = "Proxy request to %s could not resolve a
    // backend server")
    ...
    void balancerCreated(int id, String name, boolean stickySession, String stickySessionCookie,
            String stickySessionPath, boolean stickySessionRemove, boolean stickySessionForce, int waitWorker,
            int maxattempts);
    ...
    // Context path and JVMRoute redundantly logged with DEBUG soa s to provide
    // meaning for aliases.
    @LogMessage(level = DEBUG)
    ...
    void nodeConfigCreated(URI connectionURI, String balancer, String domain, String jvmRoute, boolean flushPackets,
            int flushwait, int ping, long ttl, int timeout, int maxConnections, int cacheConnections,
            int requestQueueSize, boolean queueNewRequests);
    ...
    void stuckThreadDetected(String threadName, long threadId, long active, Date start, String requestUri,
            int threshold, int stuckCount, @Cause Throwable stackTrace);
