...
public class SiteDeploymentManager {
    ...
    public synchronized SiteDeployment createSiteDeployment(DeploymentInfo servletBuilder, File webroot,
            RunwarConfigurer configurer, String deploymentKey, String vDirs, SiteOptions siteOptions) throws Exception {
        ...
        if (deployments.size() > serverOptions.autoCreateContextsMax()) {
            throw new MaxContextsException("Cannot create new servlet deployment.  The configured max is ["
                    + serverOptions.autoCreateContextsMax() + "].");
        }
        ...
        try {
            ...
            if (serverOptions.cfEngineName().toLowerCase().indexOf("adobe") == -1
                    && serverOptions.cfEngineName().toLowerCase().indexOf("boxlang") == -1) {
                ...
                if (deployments.size() == 0) {
                    ...
                    DeploymentManager manager = defaultContainer().addDeployment(servletBuilder);
                    manager.deploy();
                    deployment = new SiteDeployment(manager.start(), manager, siteOptions, serverOptions,
                            hostResourceManager);
                    this.adobeDefaultDeployment = deployment;
                    LOG.debug("  Initial servlet context created for [" + deploymentKey + "]");

                    // For all subsequent deploys, reuse default deployment and simply add new
                    // resource manager
                } else {

                    ((HostResourceManager) servletBuilder.getResourceManager()).addResourceManager(deploymentKey,
                            resourceManager);
                    // Create a new deployment and site handler chain that calls the same servlet
                    // initial handler
                    deployment = new SiteDeployment(this.adobeDefaultDeployment.getServletInitialHandler(),
                            this.adobeDefaultDeployment.getDeploymentManager(), siteOptions, serverOptions,
                            this.adobeDefaultDeployment.getResourceManager());
    ...
    public ResourceManager getResourceManager(File warFile, Long transferMinSize, Map<String, Path> aliases,
            File internalCFMLServerRoot, SiteOptions siteOptions) {
        ...
        MappedResourceManager mappedResourceManager = new MappedResourceManager(warFile, transferMinSize, aliases,
                internalCFMLServerRoot, siteOptions);
        if (!cached) {
        ...
        // 8 hours in in milliseconds-- used for both the path metadata cache AND the
        // file contents cache
        // Setting to -1 will never expire items from the cache, which is tempting-- but
        // having some sort of expiration will keep errant entries from clogging the
        // cache forever
        int METADATA_MAX_AGE = 8 * 60 * 60 * 1000;
        /*
         * DirectBufferCache.sliceSize: internally DirectBufferCache has a buffer pool.
         * This pool is responsible for allocating byte buffers to store the data that
         * is in the cache,
         * the size of those buffers will be sliceSize * slicesPerPage. Each byte buffer
         * region that is allocated in the memory is split into slicesPerPage, and then
         * each buffer
         * will have sliceSize. To give you an example, if you have 50 slicesPerPage and
         * each one is 10,000 bytes long (this would be sliceSize), each time a buffer
         * is needed,
         * it allocates a region whose size is 500,000 bytes. That region is split into
         * 50 byte buffers of length 10,000 (in bytes).
         * If those buffers are all used at some point and not reclaimed, when the pool
         * needs more buffers, it will allocate another 500,000 bytes long chunk, and so
         * on, but there is a limit to it,
         * which is maxMemory
         */
        ...
        /*
         * DirectBufferCache.maxMemory: this is the maximum number of bytes that can be
         * allocated by the pool. So, in the example above, supposed that slicesPerPage
         * is 50, and sliceSize is 10,000 bytes,
         * if you have a maxMemory of 1,000,000 bytes, it means that the buffer pool can
         * only allocate two chunks of 500,000 bytes each, because that's the number of
         * 500,000 bytes long
         * regions that fit into 1,000,000. If none of those buffers are reclaimed at
         * some point, and more buffers are needed, the buffer pool will refuse to do
         * more allocations.
         * The cache will remove the oldest entry from usage pov because it is LRU. This
         * cache is used by CachingResourceManager to store contents of files in direct
         * memory.
         */
        ...
        // I assume the memory overhead of the meta is nearly zero since it's just a
        // single POJO instance per path
        ...
        if (maxMemory > 0 && maxFileSize > 0) {
            ...
            final DirectBufferCache dataCache = new DirectBufferCache(sliceSize, slicesPerPage, maxMemory,
                    BufferAllocator.DIRECT_BYTE_BUFFER_ALLOCATOR, METADATA_MAX_AGE);

            return new CachingResourceManager(metadataCacheSize, maxFileSize, dataCache, mappedResourceManager,
                    METADATA_MAX_AGE);
        } else {
            LOG.debug("  ResourceManager file cache disabled since size is zero. Path lookups will still be cached.");
            return new CachingResourceManager(metadataCacheSize, maxFileSize, null, mappedResourceManager,
                    METADATA_MAX_AGE);
        }
    }
